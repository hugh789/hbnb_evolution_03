...
class Place(Base):
    ...
    amenities_ids_r = relationship("PlaceAmenity", back_populates="place_r")
    reviews_r = relationship("Review", back_populates="place_r")
    owner_r = relationship("User", back_populates="properties_r")

    # Note that no Amenity ids are stored in the Place record itself
    # Everything we need is in the many-to-many table
