...
@api_routes.route('/api/v1/places', methods=["POST"])
def places_post():
    """adds a new Place and returns it"""
    return Place.create()

@api_routes.route('/api/v1/places', methods=["GET"])
def places_get():
    """returns all Places"""
    return Place.all()

@api_routes.route('/api/v1/places/<place_id>', methods=["GET"])
def places_specific_get(place_id):
    """returns a specific Places"""
    return Place.specific(place_id)

@api_routes.route('/api/v1/places/<place_id>', methods=["PUT"])
