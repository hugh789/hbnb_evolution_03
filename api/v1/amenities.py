...
@api_routes.route('/api/v1/amenities', methods=["POST"])
def amenity_post():
    """ Creates a new Amenity and returns it """
    return Amenity.create()

@api_routes.route('/api/v1/amenities', methods=["GET"])
def amenity_get():
    """ Gets all Amenities """
    return Amenity.all()

@api_routes.route('/api/v1/amenities/<amenity_id>', methods=["GET"])
def amenity_specific_get(amenity_id):
    """ Gets a specific Amenity """
    return Amenity.specific(amenity_id)

@api_routes.route('/api/v1/amenities/<amenity_id>', methods=["PUT"])
