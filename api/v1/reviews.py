...
@api_routes.route('/api/v1/reviews', methods=["POST"])
def review_post():
    """ Creates a new Review and returns it """
    return Review.create()

@api_routes.route('/api/v1/reviews', methods=["GET"])
def review_get():
    """ Gets all Reviews """
    return Review.all()

@api_routes.route('/api/v1/reviews/<review_id>', methods=["GET"])
def review_specific_get(review_id):
    """ Gets a specific Review """
    return Review.specific(review_id)

@api_routes.route('/api/v1/reviews/<review_id>', methods=["PUT"])
